...
KV carries state. Referenced by task orchestration, watcher, and archiver.
...
3. Scan/filter by key pattern (watcher needs "all jobs of type X")
...
simple — the watcher's needs are modest (find jobs where `state != terminal`
...
`updated_at` is the watcher's primary query field — "find all records
