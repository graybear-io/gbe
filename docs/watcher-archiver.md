...
if acquired:
    run_sweep()
    DEL gbe:lock:watcher  # or let it expire
```
...
retention:
  # watcher enforces these via XTRIM (Redis)
...
- **Retry policy**: the watcher re-enqueues; the task layer decides what to do
