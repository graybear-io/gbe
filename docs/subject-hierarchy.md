...
└── tasks.                              # ── task orchestration ──
    ...
    └── _control.>                      # task system control plane
        ├── ...sweep                    # watcher found stuck jobs
...
gbe.tasks._control.sweep                    # watcher heartbeat
```
...
- **Q3 — Event retention**: Per-domain TTLs enforced by archive watcher. See [message-broker-notes.md](./message-broker-notes.md) for full retention table. Audit drains to cold storage before trim.
...
| `*.terminal` | `archiver` | Archive watcher for cold storage drain |
