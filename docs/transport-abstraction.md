...
| `max_age` | Periodic `XTRIM` via watcher (no native TTL on streams) |
...
- **No native stream TTL**: watcher must run `XTRIM` periodically
...
- Native per-stream retention (no watcher needed)
