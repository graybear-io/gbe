...
struct Args {
    ...
    /// Stream metadata as key=value pairs (e.g. --metadata stream_type=task-output --metadata label=output/my-task)
...
fn main() -> Result<()> {
    ...
    let metadata: std::collections::HashMap<String, String> = args.metadata.into_iter().collect();
